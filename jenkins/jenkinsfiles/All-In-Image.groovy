      ...
      steps{
        script {
          if (params.Run_all_tests == true || params.Cypress == true) {
            build(job: "Cypress_test/${env.BRANCH_NAME}", parameters: [string(name: 'BRANCH', value: "${params.BRANCH}"), string(name: 'SL_LABID', value: "${params.SL_LABID}"), string(name: 'SL_TOKEN', value: "${params.SL_TOKEN}"), string(name: 'MACHINE_DNS1', value: "${params.MACHINE_DNS}"), booleanParam(name: 'node_ci', value: true)])
          }
        }
      }
    }
