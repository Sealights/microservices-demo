...
pipeline {
   agent {
    ...
    parameters {
        ...
        string(name: 'SL_LABID', defaultValue: 'integ_Wahbibranch_f92f_BTQ@ahmad-branch-1-0-159', description: 'Lab_id')
        string(name: 'MACHINE_DNS1', defaultValue: 'http://dev-ahmad-branch-1-0-159.dev.sealights.co:8081', description: 'machine dns')
    ...
    stages{
        ...
        stage('download NodeJs agent and scanning Jest tests') {
            steps{
                script{
                    sh """
                    ...
                    #./node_modules/.bin/slnodejs jest --token  --labid  --teststage "Jest tests"  --useslnode2 -- ./test/test.js --recursive --no-timeouts 
