...
pipeline {
  ...
  parameters {
    ...
    string(name: 'BRANCH', defaultValue: 'abed/btq-staging', description: 'Branch to clone (abed/btq-staging)')
    ...
    string(name: 'BUILD_BRANCH', defaultValue: 'abed/btq-staging', description: 'Branch to Build images that have the creational LAB_ID (send to ahmad branch to build)')
...
def build_btq(Map params){
  ...
  services_list.each { service ->
    parallelLabs["${service}"] = {
      ...
      build(job: "BTQ-BUILD/abed%2Fbtq-staging", parameters: [string(name: 'SERVICE', value: "${service}"),
