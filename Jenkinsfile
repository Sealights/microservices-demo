...
pipeline {
  ...
  parameters {
    ...
    string(name: 'BRANCH', defaultValue: 'java-maven', description: 'Branch to clone (java-maven)')
    string(name: 'CHANGED_BRANCH', defaultValue: 'changed1', description: 'Branch to clone (java-maven)')
    booleanParam(name: 'enable_dd', defaultValue: false, description: 'This parameter is used for enable Datadog agent')
    string(name: 'BUILD_BRANCH', defaultValue: 'java-maven', description: 'Branch to Build images that have the creational LAB_ID (send to maven branch to build)')
